 ...
 *
 * <p>Shared because binding the components is easy to forget and fails far from its cause: every
 * {@code ItemStack} constructor throws {@code Components not bound yet} instead of anything that
 * points here.
 */
public final class TestBootstrap {
     ...
     * {@link BuiltInRegistries#DATA_COMPONENT_INITIALIZERS} while a datapack loads and only then
     * bound onto the registry holders. There is no server here to run that load, so bind them
     * against the vanilla lookup -- the same thing {@code ReloadableServerResources} does.
     */
