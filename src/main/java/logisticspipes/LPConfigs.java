...
public class LPConfigs {
    ...
    public static class Common {
        ...
        private Common(ModConfigSpec.Builder b) {
            ...
            LOGISTICS_DETECTION_LENGTH = b.comment("Max detection length for pipe network scan")
                .defineInRange("detectionLength", 50, 1, 1000);
            LOGISTICS_DETECTION_COUNT = b.comment("Max pipes counted during detection")
                .defineInRange("detectionCount", 100, 1, 10000);
            LOGISTICS_DETECTION_FREQUENCY = b.comment("Detection frequency in ticks")
                .defineInRange("detectionFrequency", 20 * 30, 1, Integer.MAX_VALUE);
            MAX_UNROUTED_CONNECTIONS = b.comment("Max unrouted connections per pipe")
                .defineInRange("maxUnroutedConnections", 32, 1, 512);
            b.pop();
            b.comment("Orderer GUI settings").push("orderer");
            LOGISTICS_ORDERER_COUNT_INVERTWHEEL = b.comment("Invert mouse wheel for item count in orderer")
                .define("invertCountWheel", false);
            LOGISTICS_ORDERER_PAGE_INVERTWHEEL = b.comment("Invert mouse wheel for page navigation in orderer")
                .define("invertPageWheel", false);
            DISPLAY_POPUP = b.comment(
                    "Set the default configuration for the popup of the Orderer Gui. Should it be used?")
                .define("displayPopup", true);
            b.pop();
            ...
            MULTI_THREAD_PRIORITY = b.comment("Worker thread priority")
                .defineInRange("threadPriority", Thread.NORM_PRIORITY, Thread.MIN_PRIORITY, Thread.MAX_PRIORITY);
            b.pop();

            b.comment("Performance settings").push(CATEGORY_PERFORMANCE);
            DISABLE_ASYNC_WORK = b.comment("Disable async work processing (use main thread only)")
                .define("disableAsyncWork", false);
            MINIMUM_INVENTORY_SLOT_ACCESS_PER_TICK = b.comment("Minimum inventory slot accesses per tick")
                .defineInRange("minSlotAccess", 10, 1, Integer.MAX_VALUE);
            MAXIMUM_INVENTORY_SLOT_ACCESS_PER_TICK = b.comment(
                    "Maximum inventory slot accesses per tick (0 = unlimited)")
                .defineInRange("maxSlotAccess", 0, 0, Integer.MAX_VALUE);
            MINIMUM_JOB_TICK_LENGTH = b.comment("Minimum ticks between async jobs")
                .defineInRange("minJobTickLength", 1, 1, Integer.MAX_VALUE);
            b.pop();
        }
    }
