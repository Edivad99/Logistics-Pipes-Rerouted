    ...
    public static PipeItemsRemoteOrdererLogistics getPipe(ItemStack stack) {
        ...
        if (level == null) {
            ...
            for (ServerLevel lvl : server.getAllLevels()) {
                if (lvl.dimension().location().hashCode() == wantHash) {
                    level = lvl;
                    break;
                }
            }
        ...
        BlockEntity tile = level.getBlockEntity(
            new BlockPos(tag.getInt("connectedPipe-x"), tag.getInt("connectedPipe-y"), tag.getInt("connectedPipe-z")));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand handIn) {
        ...
        if (pipe != null) {
            if (MainProxy.isServer(player.level())) {
                ...
                energyUse += Math.sqrt(
                    Math.pow(pipe.getX() - player.getX(), 2) + Math.pow(pipe.getY() - player.getY(), 2) + Math.pow(
                        pipe.getZ() - player.getZ(), 2));
