...
public class CraftingPipeScreen extends ModuleBaseScreen<CraftingModuleMenu> {
    ...
    public void init() {
        ...
        for (int i = 0; i < liquidCrafter; i++) {
            ...
            liquidGuiParts[i] = new AbstractButton[10];
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][0] = new SmallGuiButton(100 + 10 * i, liquidLeft + 22, topPos + 65, 10, 10,
                    "+"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][1] = new SmallGuiButton(100 + 10 * i + 1, liquidLeft + 22, topPos + 85, 10, 10,
                    "+"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][2] = new SmallGuiButton(100 + 10 * i + 2, liquidLeft + 22, topPos + 105, 10, 10,
                    "+"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][3] = new SmallGuiButton(100 + 10 * i + 3, liquidLeft + 22, topPos + 125, 10, 10,
                    "+"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][4] = new SmallGuiButton(100 + 10 * i + 4, liquidLeft + 8, topPos + 65, 10, 10,
                    "-"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][5] = new SmallGuiButton(100 + 10 * i + 5, liquidLeft + 8, topPos + 85, 10, 10,
                    "-"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][6] = new SmallGuiButton(100 + 10 * i + 6, liquidLeft + 8, topPos + 105, 10, 10,
                    "-"))));
            extension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(
                liquidGuiParts[i][7] = new SmallGuiButton(100 + 10 * i + 7, liquidLeft + 8, topPos + 125, 10, 10,
                    "-"))));
            if (isAdvancedSat) {
            ...
            for (int i = 0; i < 9; i++) {
                if (craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i).isEmpty()) {
                    ...
                    if (!satellitePipeLabels[i].isTextEqual(
                        craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i))) {
                        satellitePipeLabels[i].setText(
                            craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i));
                    }
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int x, int y, float f) {
        LPGuiGraphics.drawGuiBackGround(guiGraphics,
            leftPos, topPos, leftPos + panelWidth - (hasByproductExtractor ? 40 : 0), topPos + panelHeight, 0.0f, true,
            true, true, true, true);
    ...
    private final class FluidCraftingExtension extends GuiExtension {
            ...
            if (isFullyExtended()) {
                ...
                if (!isAdvancedSat) {
                    ...
                    if (craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty()) {
                        ...
                        guiGraphics.text(minecraft.font, craftingModule.clientSideSatelliteNames.liquidSatelliteName(),
                            left + (liquidCrafter * 40) / 2 + 3 - (
                                font.width(craftingModule.clientSideSatelliteNames.liquidSatelliteName()) / 2),
                            top + 145, 0xFF404040, false);
                        for (int i = 0; i < liquidCrafter; i++) {
                            for (int j = 0; j < 8; j++) {
        ...
        private void renderFluidText(GuiGraphicsExtractor guiGraphics, int left, int top, int i) {
            ...
            guiGraphics.text(minecraft.font, liquidAmount, left + 22 - (font.width(liquidAmount) / 2), top + 40,
                0xFF404040, false);
            ...
            if (isAdvancedSat) {
                if (craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(i).isEmpty()) {
                    ...
                    guiGraphics.text(minecraft.font, name, left + 22 - (font.width(name) / 2), top + 146, 0xFF404040,
                        false);
                    for (int j = 0; j < 8; j++) {
                        liquidGuiParts[i][j].active = true;
                    }
            ...
            if (craftingModule.liquidInventory.getItem(i).isEmpty() && !(
                (!isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty()) || (
                    isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(i)
                        .isEmpty()))) {
        ...
        public boolean renderSelectSlot(int slotId) {
            if ((isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(id).isEmpty()) || (
                !isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty())) {
                return false;
            }
