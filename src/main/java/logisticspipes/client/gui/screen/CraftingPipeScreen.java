...
public class CraftingPipeScreen extends ModuleBaseScreen<CraftingModuleMenu> {
	...
	public CraftingPipeScreen(CraftingModuleMenu menu, Inventory inventory, Component title) {
	...
	public void init() {
		...
		if (!isAdvancedSat) {
			...
			addRenderableWidget(normalButtonArray[0] = new SmallGuiButton(0, (width - panelWidth) / 2 + 125, (height - panelHeight) / 2 + 57, 37, 10, TextUtil.translate(PREFIX + "Select")));
			normalButtonArray[0].active = craftingModule.getSlot().isInWorld();
			addRenderableWidget(normalButtonArray[1] = new SmallGuiButton(3, (width - panelWidth) / 2 + 39, (height - panelHeight) / 2 + 50, 37, 10, TextUtil.translate(CraftingPipeScreen.PREFIX + "Import")));
			addRenderableWidget(normalButtonArray[2] = new SmallGuiButton(4, (width - panelWidth) / 2 + 6, (height - panelHeight) / 2 + 50, 28, 10, TextUtil.translate(CraftingPipeScreen.PREFIX + "Open")));
			addRenderableWidget(normalButtonArray[3] = new SmallGuiButton(20, (width - panelWidth) / 2 + 155, (height - panelHeight) / 2 + 85, 10, 10, ">"));
			addRenderableWidget(normalButtonArray[4] = new SmallGuiButton(21, (width - panelWidth) / 2 + 120, (height - panelHeight) / 2 + 85, 10, 10, "<"));
			if (liquidCrafter != 0) {
			...
			addRenderableWidget(normalButtonArray[1] = new SmallGuiButton(3, (width - panelWidth) / 2 + 39, (height - panelHeight) / 2 + 100, 37, 10, TextUtil.translate(CraftingPipeScreen.PREFIX + "Import")));
			...
			addRenderableWidget(normalButtonArray[3] = new SmallGuiButton(20, (width - panelWidth) / 2 + 155, (height - panelHeight) / 2 + 105, 10, 10, ">"));
			addRenderableWidget(normalButtonArray[4] = new SmallGuiButton(21, (width - panelWidth) / 2 + 120, (height - panelHeight) / 2 + 105, 10, 10, "<"));
		}
		for (int i = 0; i < liquidCrafter; i++) {
			if (isAdvancedSat) {
		...
		if (cleanupSize > 0) {
			...
			cleanupExtension.registerButton(extensionControllerLeft.registerControlledButton(addRenderableWidget(cleanupModeButton = new SmallGuiButton(24, leftPos - 56, topPos + 18 + (18 * cleanupSize), 50, 10, TextUtil.translate(CraftingPipeScreen.PREFIX + (
					cleanupModeIsExcludeOverlay.get() ? "Exclude" : "Include"))))));
			cleanupExtension.registerButton(
				extensionControllerLeft.registerControlledButton(addRenderableWidget(new SmallGuiButton(25, leftPos - 56, topPos + 32 + (18 * cleanupSize), 50, 10, TextUtil.translate(CraftingPipeScreen.PREFIX + "Import")))));
			for (int i = 0; i < cleanupSize * 3; i++) {
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Inputs"), 18, 7, 0xFF404040, false);
		guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Inventory"), 10, panelHeight - 93, 0xFF404040, false);

		if (!isAdvancedSat) {
			guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Output"), 77, 40, 0xFF404040, false);
			guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Satellite"), 123, 7, 0xFF404040, false);
			if (craftingModule.clientSideSatelliteNames.satelliteName().isEmpty()) {
				guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Off"), 135, 43, 0xFF404040, false);
			} else {
			...
			guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Priority") + ":", 123, 75, 0xFF404040, false);
			guiGraphics.text(minecraft.font, String.valueOf(craftingPriorityOverlay.get()), 143 - (minecraft.font.width(String.valueOf(craftingPriorityOverlay.get())) / 2), 87, 0xFF404040, false);
		} else {
			for (int i = 0; i < 9; i++) {
				if (craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i).isEmpty()) {
					guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Off"), 9 + (i * 18), 57, 0xFF404040, false);
				} else {
					if (!satellitePipeLabels[i].isTextEqual(craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i))) {
						satellitePipeLabels[i].setText(craftingModule.clientSideSatelliteNames.advancedSatelliteNames().get(i));
					}
					satellitePipeLabels[i].draw(guiGraphics, mouseX - leftPos, mouseY - topPos);
				}
			}
			guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Output"), 77, 90, 0xFF404040, false);
			guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Priority") + ":", 123, 95, 0xFF404040, false);
			guiGraphics.text(minecraft.font, String.valueOf(craftingPriorityOverlay.get()), 143 - (minecraft.font.width(String.valueOf(craftingPriorityOverlay.get())) / 2), 107, 0xFF404040, false);
		}
	}
	...
	protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int x, int y, float f) {
		LPGuiGraphics.drawGuiBackGround(guiGraphics,
                leftPos, topPos, leftPos + panelWidth - (hasByproductExtractor ? 40 : 0), topPos + panelHeight, 0.0f, true, true, true, true, true);
		...
		LPGuiGraphics.drawPlayerInventoryBackground(guiGraphics, leftPos + 8, topPos + panelHeight - 82);
	...
	private final class FluidCraftingExtension extends GuiExtension {
			...
			if (isFullyExtended()) {
				...
				if (!isAdvancedSat) {
					...
					if (craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty()) {
                        ...
                        guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Off"), left + (liquidCrafter * 40) / 2 - 5, top + 145, 0xFF404040, false);
						for (int i = 0; i < liquidCrafter; i++) {
							for (int j = 0; j < 8; j++) {
                        ...
                        guiGraphics.text(minecraft.font, craftingModule.clientSideSatelliteNames.liquidSatelliteName(), left + (liquidCrafter * 40) / 2 + 3 - (font.width(craftingModule.clientSideSatelliteNames.liquidSatelliteName()) / 2), top + 145, 0xFF404040, false);
						for (int i = 0; i < liquidCrafter; i++) {
							for (int j = 0; j < 8; j++) {
		...
		private void renderFluidText(GuiGraphicsExtractor guiGraphics, int left, int top, int i) {
			...
			guiGraphics.text(minecraft.font, liquidAmount, left + 22 - (font.width(liquidAmount) / 2), top + 40, 0xFF404040, false);
			guiGraphics.text(minecraft.font, "1", left + 19, top + 53, 0xFF404040, false);
			guiGraphics.text(minecraft.font, "10", left + 16, top + 73, 0xFF404040, false);
			guiGraphics.text(minecraft.font, "100", left + 13, top + 93, 0xFF404040, false);
			guiGraphics.text(minecraft.font, "1000", left + 10, top + 113, 0xFF404040, false);
			if (isAdvancedSat) {
				if (craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(i).isEmpty()) {
					...
					guiGraphics.text(minecraft.font, TextUtil.translate(CraftingPipeScreen.PREFIX + "Off"), left + 15, top + 146, 0xFF404040, false);
					for (int j = 0; j < 8; j++) {
						liquidGuiParts[i][j].active = false;
					}
				} else {
					String name = craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(i);
					name = TextUtil.getTrimmedString(name, 40, minecraft.font, "...");
					guiGraphics.text(minecraft.font, name, left + 22 - (font.width(name) / 2), top + 146, 0xFF404040, false);
					for (int j = 0; j < 8; j++) {
						liquidGuiParts[i][j].active = true;
					}
				}
			...
			if (craftingModule.liquidInventory.getItem(i).isEmpty() && !((!isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty()) || (isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(i).isEmpty()))) {
		...
		public boolean renderSelectSlot(int slotId) {
			if ((isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteNames().get(id).isEmpty()) || (!isAdvancedSat && craftingModule.clientSideSatelliteNames.liquidSatelliteName().isEmpty())) {
				return false;
			}
