...
public class FluidSupplierModuleScreen extends ModuleBaseScreen<SimpleFilterMenu> {
    ...
    protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        guiGraphics.text(minecraft.font,
            ((logisticspipes.utils.item.ItemIdentifierInventory) liquidSupplier.getFilterInventory()).getName(), 8, 6,
            0xFF404040, false);
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int x, int y, float f) {
        ...
        int j = leftPos;
        int k = topPos;
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, FluidSupplierModuleScreen.TEXTURE, j, k, 0.0f, 0.0f, panelWidth,
            panelHeight, 256, 256);
    }
}
