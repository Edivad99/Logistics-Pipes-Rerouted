...
public class PowerJunctionScreen extends LogisticsBaseGuiScreen<PowerJunctionMenu> {
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.text(minecraft.font, TextUtil.translate(PowerJunctionScreen.PREFIX + "LogisticsPowerJunction"), 30, 8, 0xFF404040, false);
		guiGraphics.text(minecraft.font, TextUtil.translate(PowerJunctionScreen.PREFIX + "StoredEnergy") + ":", 40, 23, 0xFF404040, false);
	...
	protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int var2, int var3, float var1) {
		int j = leftPos;
		int k = topPos;
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, PowerJunctionScreen.TEXTURE, j, k, 0.0f, 0.0f, panelWidth, panelHeight, 256, 256);
		int level = 100 - junction.getChargeState();
		int levelPixels = level * 59 / 100;
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, PowerJunctionScreen.TEXTURE, j + 10, k + 11 + levelPixels, 176.0f, levelPixels, 5, 59 - levelPixels, 256, 256);
	}
