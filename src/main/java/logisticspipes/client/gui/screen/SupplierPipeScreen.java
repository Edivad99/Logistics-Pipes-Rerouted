...
public class SupplierPipeScreen extends LogisticsBaseGuiScreen<ActiveSupplierMenu> {
        ...
        if (hasPatternUpgrade) {
            slotAssignmentPatternOverlay.read((slotAssignments) -> {
                for (int i = 0; i < slotAssignments.size(); i++) {
                    guiGraphics.text(minecraft.font, Integer.toString(slotAssignments.get(i)), 22 + i * 18, 55,
                        0xFF404040, false);
                }
                return null;
            });
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int x, int y, float f) {
        if (!hasPatternUpgrade) {
            ...
            int j = leftPos;
            int k = topPos;
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, SupplierPipeScreen.TEXTURE, j, k, 0.0f, 0.0f, panelWidth,
                panelHeight, 256, 256);
        } else {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (slot.hasItem() && slot.getItem().getCount() > 64) {
                    guiGraphics.fill(leftPos + 18 + i * 18, topPos + 20, leftPos + 34 + i * 18, topPos + 36,
                        Color.getValue(Color.RED));
                }
            }
