...
public class ActionChoicePopup extends SubGuiScreen {
    ...
    public ActionChoicePopup(String message, String leftButton, Runnable leftAction, String rightButton,
        ...
        int sizeX = font.width(message);
        int leftX = font.width(leftButton);
        int rightX = font.width(rightButton);
    ...
    public void init() {
        ...
        SmallGuiButton lBtn = new SmallGuiButton(0, lX, guiTop + 25, lW, 20, leftButton);
        lBtn.setPressListener(b -> {
            leftAction.run();
            exitGui();
        });
        addRenderableWidget(lBtn);
        SmallGuiButton rBtn = new SmallGuiButton(1, rX, guiTop + 25, rW, 20, rightButton);
        rBtn.setPressListener(b -> {
            rightAction.run();
            exitGui();
        });
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.text(minecraft.font, message, xCenter - minecraft.font.width(message) / 2, guiTop + 6, 0xFFFFFFFF,
            true);
    }
